...
mod tests {
    ...
    fn test_hash_and_verify_with_pepper() {
        ...
        assert!(
            !verify_password(password, &hashed, "wrong_pepper")
                .expect("Verify should fail with wrong pepper")
        );
        assert!(
            !verify_password("wrong_password", &hashed, pepper)
                .expect("Verify should fail with wrong password")
        );
    }
}
